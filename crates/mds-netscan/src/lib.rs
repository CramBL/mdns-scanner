...
impl NetworkScanner {
    const MIN_THREADS_PER_SCAN: NonZero<u16> = NonZero::new(10).unwrap();
    ...
    fn run(&mut self) {
        while !self.stop_flag.load(Ordering::Relaxed) {
            ...
            let threads_per_scan = self.threads_per_scan(num_iface);
            if num_iface.get() == 1 {
                log::debug!("Network scan will use at most {threads_per_scan} I/O threads");
            } else {
                let threads_per_iface = threads_per_scan.get() / num_iface.get();
                log::debug!(
