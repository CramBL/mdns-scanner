...
impl PortScanPopup {
    ...
    /// Right-aligned column for the header's total, wide enough for `65535 ports`.
    const TOTAL_COLUMN_WIDTH: usize = "65535 ports".len();
    ...
    /// The scanning progress gauge sits at the top of the content, its open-port
    /// list two rows below (one blank separator).
    const SCANNING_GAUGE_ROW: u16 = Self::CONTENT_TOP;
    const SCANNING_LIST_ROW: u16 = Self::CONTENT_TOP + 2;
    /// Content rows above the scanning open-port list: the gauge row and one
    /// blank separator.
    ...
    /// The results summary and counts lines plus one blank separator, above the
    /// grouped open-port view.
    const RESULTS_FIXED_LINE_COUNT: u16 = 3;
    ...
    /// Braille cycle for the indeterminate spinner shown before the first progress
    /// update lands.
        ...
        match &mut self.state {
            ...
            PortScanState::Scanning(running) => {
                if running.scanned == 0 {
                    Self::render_starting_spinner(running, interior, frame, theme);
                } else {
                    Self::render_progress_gauge(running, interior, frame, theme);
                }
            ...
            PortScanState::Results(result) => {
                ...
                let area = Rect {
                    ...
                    height: content.len().clamp(1, Self::MAX_VISIBLE_RESULT_ROWS) as u16,
                    ..interior
                };
    ...
    /// The header's three fixed-height lines: the scanned ranges with their total
    /// port count, the worker/timeout/max-duration metrics, and a status line. The
    /// height is fixed across every state so nothing below it shifts.
        ...
        let (ranges, metrics) = match settings.mdns_probe_timeout {
            Some(timeout) if settings.port_count == 0 => (
                ...
                format!(
                    ...
                    duration = util::format_duration(timeout),
                ),
            ),
            _ => {
                let max = if settings.port_count == 0 {
                    ...
                    util::format_duration(settings.all_filtered_duration())
                };
                (
                    Self::ranges_line(&settings.port_ranges, settings.port_count, width),
                    format!(
                        "{workers:<ww$}{timeout:<tw$}max {max}",
                        workers = format!("{n} workers", n = settings.worker_count.get()),
                        timeout = util::format_duration(settings.connect_timeout),
    ...
    /// The scanned ranges left-aligned with `{n} ports` right-aligned to `width`,
    /// truncating the ranges with `…` if they would collide with the total.
    fn ranges_line(port_ranges: &PortRanges, port_count: usize, width: usize) -> String {
        let total = format!("{port_count} ports");
        let ranges_budget = width.saturating_sub(Self::TOTAL_COLUMN_WIDTH + 1);
        let ranges = Self::truncate_with_ellipsis(&port_ranges.to_string(), ranges_budget);
        let gap = width.saturating_sub(ranges.width() + total.width());
        format!("{ranges}{spaces}{total}", spaces = " ".repeat(gap))
    }
    ...
    /// The results summary and one blank separator, then placeholder rows for the
    /// grouped open-port view rendered over them.
    fn results_placeholder_lines(
        ...
        let header = format!(
            "{outcome} {duration}",
            outcome = if result.was_cancelled() {
                "Cancelled after"
            } else {
                "Done in"
            },
        ...
        lines.push(Line::raw(""));
        let content_rows = content_len.clamp(1, Self::MAX_VISIBLE_RESULT_ROWS);
    ...
    fn render_progress_gauge(
        running: &RunningPortScan,
        interior: Rect,
        frame: &mut Frame,
        theme: &TableColors,
    ) {
        let scanned = running.scanned;
        let total = running.settings.port_count;
        ...
        frame.render_widget(
            gauge,
            Rect {
                y: interior.y + Self::SCANNING_GAUGE_ROW,
                height: 1,
                ..interior
            },
        );
    ...
    /// The spinner glyph for a given elapsed time, cycled from the braille frames.
    fn spinner_frame(elapsed: Duration) -> &'static str {
        let index = (elapsed.as_millis() / Self::SPINNER_FRAME_INTERVAL_MS) as usize
            % Self::SPINNER_FRAMES.len();
        Self::SPINNER_FRAMES
            .get(index)
            .copied()
            .unwrap_or(Self::SPINNER_FRAMES[0])
    }

    /// Draws an indeterminate spinner where the gauge will go, shown while the scan
    /// has started but no port has reported yet.
    fn render_starting_spinner(
        running: &RunningPortScan,
        interior: Rect,
        frame: &mut Frame,
        theme: &TableColors,
        ...
        let line = Line::from(vec![
            Span::styled(
                Self::spinner_frame(running.started.elapsed()),
                theme.gauge_accent().add_modifier(Modifier::BOLD),
            ),
            Span::styled(" starting scan", theme.row().add_modifier(Modifier::DIM)),
        ]);
        frame.render_widget(
            Paragraph::new(line),
            Rect {
