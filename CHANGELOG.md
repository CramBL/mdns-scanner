...
- Port scanning of the host selected in the table: choose from the standard port ranges or a custom list, optionally check the multicast DNS port, and watch the open ports appear grouped by category. A scan can be cancelled or rerun, and each host keeps its last result, shown in the info popup.
