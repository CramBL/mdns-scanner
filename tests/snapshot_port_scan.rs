...
fn test_settings_header_is_identical_across_states() {
    ...
    1-1023, 49152-65535                 17407 ports
...
/// Before the first port reports, the gauge row shows an indeterminate spinner.
/// The stubbed scan's fixed elapsed time pins the spinner to its first frame.
#[test]
fn test_port_scan_shows_a_spinner_before_the_first_progress_update() {
    let mut h = harness_with_one_host();
    h.run(Action::PortScan);
    h.model
        .start_stubbed_port_scan(vec![IanaPortCategory::System]);
